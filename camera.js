...
import {drawBoundingBox, drawKeypoints, calculateAngle, drawSkeleton, isMobile,
  toggleLoadingUI, tryResNetButtonName,
  ...
  async function poseDetectionFrame() {
    ...
    poses.forEach(({score, keypoints}) => {
      if (score >= minPoseConfidence) {
        let wantedPoints = [6, 8, 10];

        calculateAngle(wantedPoints, keypoints);

        if (guiState.output.showPoints) {
          drawKeypoints(keypoints, minPartConfidence, ctx, wantedPoints);
        }
